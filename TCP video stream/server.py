...
ip="192.168.133.232"   # Server public IP
port=2323           # Server port number
# Bind the IP:port to connect 
tcp_sock.bind(("192.168.133.232", 2323))
# Listening To Connections(clients), this typically used by server, where 
# "listen()" means can have multiple, Even we c an limit client like this "listen(Number_of_clients)" Ex.-> "listen(5)"
tcp_sock.listen()
